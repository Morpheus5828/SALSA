...
images = resize.resize_to_same_width(images, average_width)'''


'''# Save features
features, descriptors = extract.compute(images, extract.get_sift_descriptor)

extract.save_features("sift-sea-features.txt", "save/features/sift/sea/features", features)
extract.save_descriptors("sift-sea-descriptors.txt", "save/features/sift/sea/descriptors", descriptors)'''

'''# Load features
features = extract.read_features("save/features/sift/sea/features/resized_a-sift-sea-features.txt", float)
#descriptors = extract.read_descriptors("save/features/sift/sea/descriptors/resized_a-sift-sea-descriptors.txt", float)
print(int(np.log(len(features))))

# Save clustering algorithm
clustering_model = clustering.get_gaussian_mixture_model(features, int(np.log(len(features))))
clustering.save_model(clustering_model, "resized_a-gm-sift-sea-12.joblib", "save/clustering/gaussian-mixture/sift")'''


# Load clustering algorithm
clustering_model = clustering.load_model("save/clustering/gaussian-mixture/sift/resized_a-gm-sift-sea-12.joblib")
nb_clusters = 12


# Compute histogram
all_descriptors = extract.read_descriptors("save/features/sift/all/descriptors/resized_a-sift-all-descriptors.txt", float)
